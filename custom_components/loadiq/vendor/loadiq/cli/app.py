...
from ..data.factory import create_power_data_source
...
def _load_segments(cfg: LoadIQConfig, start: pd.Timestamp, end: pd.Timestamp) -> list[DetectedSegment]:
    _suppress_missing_pivot_warnings()
    try:
        data_source = create_power_data_source(cfg)
    except NotImplementedError as exc:
        raise click.ClickException(str(exc)) from exc
    except Exception as exc:  # pragma: no cover - factory failures are rare
        raise click.ClickException(f"Failed to initialise data backend '{cfg.backend.value}': {exc}") from exc
    ...
    try:
        ...
        if cfg.backend == DataBackend.INFLUXDB and cfg.influx:
            raise click.ClickException(
                f"Could not reach InfluxDB at {cfg.influx.url}: {exc}"
            ) from exc
        raise click.ClickException(
            f"Data backend '{cfg.backend.value}' raised an unexpected error: {exc}"
        ) from exc
