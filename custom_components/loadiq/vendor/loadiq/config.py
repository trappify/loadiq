...
class DataBackend(str, Enum):
    """Supported data backends for fetching power series."""
...
class LoadIQConfig(BaseModel):
    ...
    backend: DataBackend = Field(default=DataBackend.INFLUXDB)
    influx: Optional[InfluxConnection] = None
    entities: EntitiesConfig
    detection: DetectionConfig = Field(default_factory=DetectionConfig)

    @model_validator(mode="after")
    def _ensure_backend_requirements(self) -> "LoadIQConfig":
        if self.backend == DataBackend.INFLUXDB and self.influx is None:
            raise ValueError("InfluxDB backend selected but no 'influx' configuration provided.")
        return self
    ...
    def from_env(cls) -> "LoadIQConfig":
        ...
        import os
        backend_value = os.getenv("LOADIQ_BACKEND", DataBackend.INFLUXDB.value)
        ...
        influx_cfg: Optional[InfluxConnection] = None
